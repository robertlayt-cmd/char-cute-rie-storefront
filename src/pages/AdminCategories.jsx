...
import { Plus, Pencil, Trash2, Upload, ChevronDown, ChevronRight, Check } from 'lucide-react';
...
export default function AdminCategories() {
  ...
  const setupHierarchy = useMutation({
    mutationFn: async () => {
      const { data } = await base44.functions.invoke('setupCategoryHierarchy');
      return data;
    },
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ['admin-categories'] });
  ...
  const moveOrder = async (category, direction) => {
    ...
    await Promise.all([
      updateCategory.mutateAsync({ 
        id: category.id, 
        data: { display_order: swap.display_order ?? swapIdx } 
      }),
      updateCategory.mutateAsync({ 
        id: swap.id, 
        data: { display_order: category.display_order ?? idx } 
      }),
    ]);
  };
          ...
          <CardContent className="p-3 flex items-center gap-3">
            ...
            <div className="flex gap-1">
              ...
              <Button 
                variant="outline" 
                size="sm" 
                className="border-zinc-700 h-7 w-7 p-0" 
              ...
              <Button
                ...
                onClick={() => { 
                  if (confirm('Delete category?')) deleteCategory.mutate(category.id); 
                }}
              >
      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between gap-4 mb-8">
          ...
          <div className="flex gap-2">
            <Button
              onClick={() => setupHierarchy.mutate()}
              disabled={setupHierarchy.isPending}
              className="bg-amber-600 hover:bg-amber-700"
              title="Auto-setup category hierarchy"
            >
              {setupHierarchy.isPending ? 'Setting up...' : 'Setup Hierarchy'}
            </Button>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(v) => setEditingCategory({ ...editingCategory, parent_id: v === 'none' ? null : v })}
                  >
                ...
                <div>
                  <Label className="text-zinc-200">Slug</Label>
                  <Input
